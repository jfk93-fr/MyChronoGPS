...
###########################################################################
#
# MyChronoGPS_TEInkPaper_DB
#   control of the displays on a Waveshare Touch e-Paper HAT screen 
#   reads the cached file DASHBOARD, formats the message and displays it on the screen
...
cmdgps =  "MyChronoGPS_TEInkPaper_DB"
...
class ScrollControl(threading.Thread):
    def __init__(self,max):
        ...
        self.light = 1
    def run(self):
        logger.info("thread ScrollControl running")
        self.__running = True
        while self.__running:
            ...
            #    self.light = 1
            time.sleep(0.3)
        logger.info("thread ScrollControl ended")
    def stop(self):
...
#######################################################################
    ...
    def lire_cache(self):
        ...
        with open(self.cache, 'r') as cache:
            message = cache.read()
            if self.debug == 0:
                logger.debug(str(self.cache)+" ["+str(message)+"]")
        try:
            mydict = json.loads(message)
        except:
            if self.debug == 0:
                logger.debug("error json load "+str(self.cache)+" ["+str(message)+"]")
                self.debug = 1
            pass
            return True
        ...
        if self.loopm > 100:
            ...
            if self.circuit != mydict["circuit"]:
                ...
                self.displayVar("circuit")
            if self.date != mydict["date"] and mydict["date"] != "":
                self.date = mydict["date"]
                self.displayVar("date")
        self.loopm = self.loopm+1

        #self.draw.rectangle((0,0,self.height,15), outline=0, fill=255)
        #self.draw.rectangle((0,16,self.height,31), outline=0, fill=255)
        #self.draw.rectangle((0,32,self.height,self.width-24), outline=0, fill=255)
        ...
        if self.vitesse != mydict["vitesse"]:
            ...
            self.displayVar("vitesse")

        if self.display2var == True:
            self.epd.displayPartial(self.epd.getbuffer(self.image))
            self.epd.ReadBusy()
            self.display2var = False
        time.sleep(0.09)
        return True

    def displayVar(self,var):
        ...
        if var == "infos":
            ...
            if self.infos == " " and self.infos != self.previnfos:
                ...
                lib = self.scr.get_scroll(self.infos)
            font = ImageFont.truetype(self.ttf,14)
            self.draw.rectangle((0,self.width-24,self.height,self.width), outline=0, fill=255)
            self.draw.text((4, self.width-24), lib,  font=font, fill=0)
            #self.epd.displayPartial(self.epd.getbuffer(self.image))
            #self.epd.ReadBusy()
            self.display2var = True
            return
        if var == "nbsats":
            lib = str(self.nbsats)
            font = ImageFont.truetype(self.ttf,10)
            self.draw.rectangle((0,0,8,15), outline=0, fill=255)
            self.draw.text((2, 2), lib,  font=font, fill=0)
            #self.epd.displayPartial(self.epd.getbuffer(self.image))
            #self.epd.ReadBusy()
            self.display2var = True
            return
        if var == "date":
            lib = str(self.date)
            font = ImageFont.truetype(self.ttf,10)
            self.draw.rectangle((8,0,70,15), outline=0, fill=255)
            self.draw.text((16, 2), lib,  font=font, fill=0)
            #self.epd.displayPartial(self.epd.getbuffer(self.image))
            #self.epd.ReadBusy()
            self.display2var = True
            return
        if var == "time":
            lib = str(self.time)
            font = ImageFont.truetype(self.ttf,10)
            self.draw.rectangle((71,0,120,15), outline=0, fill=255)
            self.draw.text((78, 2), lib,  font=font, fill=0)
            #self.epd.displayPartial(self.epd.getbuffer(self.image))
            #self.epd.ReadBusy()
            self.display2var = True
            return
        if var == "ip":
            lib = str(self.ip)
            font = ImageFont.truetype(self.ttf,10)
            self.draw.rectangle((121,0,180,15), outline=0, fill=255)
            self.draw.text((128, 2), lib,  font=font, fill=0)
            #self.epd.displayPartial(self.epd.getbuffer(self.image))
            #self.epd.ReadBusy()
            self.display2var = True
            return
    ...
    def boucle(self):
        ...
        while self.running == True:
            ...
            if(self.stateDisplay == 0 and self.touch() == False):
                self.running = self.lire_cache()
                #logger.debug("running:"+str(self.running))
            else:
                self.dialog()

        self.draw.rectangle((0,0,self.height,self.width), outline=0, fill=255)
        self.draw.text((5, 5), "end of",  font=self.font, fill=0)
        self.draw.text((5, 60), "MyChronoGPS",  font=self.font, fill=0)
        self.epd.displayPartial(self.epd.getbuffer(self.image))
        self.epd.ReadBusy()        
        time.sleep(5)
...
if __name__ == '__main__':
    try:
        logger.info("MyChronoGPS_TEInkPaper_DB")
        ...
        logger.debug("début boucle running")
        ...
        print("END MyChronoGPS_TEInkPaper_DB")